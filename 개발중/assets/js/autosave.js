        ...
        $.get(g5_bbs_url + "/ajax.autosavelist.php", function (data) {
            ...
            if ($(data).find("list").find("item").length > 0) {
                $(data).find("list").find("item").each(function (i) {
                    ...
                    $("#autosave_pop ul")
                        .append('<li class="list-group-item"><a href="#none" class="autosave_load">' + subject + '</a><span>' + datetime + ' <button type="button" class="btn btn-sm btn-outline-danger autosave_del"><i class="bi bi-x-circle-fill"></i></button></span></li>')
                        .find("li:eq(" + i + ")")
