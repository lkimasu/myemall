...
<?php if ($is_comment_write) {
    ...
    <aside id="bo_vc_w">
        <h2>댓글쓰기</h2>
        <form name="fviewcomment" action="/bbs/write_comment_update.php" onsubmit="return fviewcomment_submit(this);" method="post" autocomplete="off">
        ...
        <div class="tbl_frm01 tbl_wrap">
			...
			<textarea id="wr_content" name="wr_content" required title="댓글 내용" placeholder="  권리침해, 욕설, 특정 대상을 비하하는 내용을 게시할 경우 이용약관 및 관련 법률에 의해 제재될 수 있습니다."
			<?php if ($comment_min || $comment_max) { ?>onkeyup="check_byte('wr_content', 'char_count');"<?php } ?>><?php echo $c_wr_content; ?></textarea>
			<?php if ($comment_min || $comment_max) { ?><script> check_byte('wr_content', 'char_count'); </script><?php } ?>
			<div id="cmt_guset">
				<div class="cmt_guset_nomem">
					<label for="wr_secret">비밀글사용</label>
		            <input type="checkbox" name="wr_secret" value="secret" id="wr_secret">
		            
					<?php if ($is_guest) { ?>
		            <label for="wr_name">이름<strong class="sound_only">필수</strong></label>
		            <input type="text" name="wr_name" id="wr_name" required class="frm_input required" size="5" maxLength="20">
				...
				<div class="cmt_guset_auto">
					<?php if ($is_guest) { ?>
		            자동등록방지<?php echo $captcha_html; ?>
		            <?php } ?>
		            ...
		            SNS 동시등록<div id="bo_vc_send_sns"></div>
	        ...
	        <div class="btn_confirm">
	            <button type="submit" id="btn_submit" class="btn_submit" accesskey="s"><i class="fa fa-plus" aria-hidden="true"></i> 댓글등록</button>
	        </div>
		</div>
        </form>
    </aside>
